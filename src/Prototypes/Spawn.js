        ...
        if (name == undefined) {
            for (let role of Common.general) {
                ...
                if (role == 'starter' && this.memory.startRoom != undefined) {
                    ...
                    if (starters < 1) {
                        name = this.createStart(this.memory.startRoom, this.room.name);
                        if (name != undefined && _.isString(name)) {
                            delete this.memory.startRoom;
                        }
                        break;
                    }
                }
...
StructureSpawn.prototype.createStarter =
    function (target, home) {
        var body = Common.roles["starter"].parts(false);
        var name = "Starter"+Game.time;
        if (this.spawnCreep(body, name, {memory: {role: 'starter', working: false, target: target, home: home}}) == OK) {
            return name;
        }        
    }
